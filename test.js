...
async function findAnswer(userInput) {
  ...
  if (
    ...
    !coveringTypes.some((type) => normalizedUserInput.includes(type))
  ) {
    return {
  ...
  // Step 6: 제품 정보(goodsInfo) 관련 조건 - Levenshtein 비교
  ...
  // Step 8: 배송정보(deliveryInfo) 관련 조건 - Levenshtein 비교
  ...
  // Step 10: 그 외, covering 및 biz 영역의 Levenshtein 비교 (최종 비교)
  let bestMatch = null;
  ...
  if (bestCategory === "covering") {
    const videoUrl = bestMatch.videoHtml ? bestMatch.videoHtml : null;
    return {
...
app.post("/chat", async (req, res) => {
  ...
  try {
    ...
    console.error("Error in findAnswer function:", error.message);
